...
1. **`catch_up_on_boot=True`** AND the job qualifies under ONE of two admissible motivations:
   - **(a) Safety-net, missed = lost forever:** a missed cadence window can never be re-fetched cleanly (e.g. yesterday's daily-index). The carve-out prevents the boot-time-only `catch_up` evaluation trap (a prereq-blocked catch-up cannot re-fire when bootstrap completes later).
   - **(b) Operator-visible live state hidden by bootstrap (#1435):** the job produces current-state data the operator legitimately expects to see *during* the multi-hour bootstrap (e.g. broker portfolio / FX). The data is technically recoverable (re-fetched each cadence), but gating it behind bootstrap completion makes the dashboard blank for hours and leaves the broker credential unexercised. `catch_up_on_boot=True` ensures it fires at boot rather than waiting for the first scheduled tick.
...
3. **Empty-universe behavior is a natural no-op:** body is idempotent + safe against an empty/partial DB. No destructive write, no expensive fetch loop. For composite layer-runners (b), each constituent layer must itself self-skip cleanly until its own init/credential preconditions are met (no error, no FK violation).
4. **Cost is bounded:** one fetch per fire, sized in single-digit MB. Anything larger needs operator-intentional gating.

Members:

- `sec_daily_index_reconcile` (motivation a) — Layer 2 of the #863-#873 ETL freshness redesign. One ~1MB daily-index fetch; `subject_resolver` filters every unknown CIK to a natural no-op; `prerequisite=None`. Missed cadence = lost-forever reconcile.
- `orchestrator_high_frequency_sync` (motivation b, #1435) — every-5-min refresh of the `portfolio_sync` + `fx_rates` layers. Empty-DB-safe: `portfolio_sync` carries `requires_layer_initialized=("universe",)` so the executor `PREREQ_SKIP`s it (clean — recorded skip, no error) until the eToro universe is committed; `fx_rates` is independent of the SEC universe (Frankfurter API, no credential gate). `prerequisite=None`. Bounded cost (broker positions + FX, single-digit MB). FK safety is preserved by the layer-init gate (FK-safe once `nightly_universe_sync` commits the universe transaction atomically; otherwise PREREQ_SKIP), NOT by the universal bootstrap gate — so the dashboard populates as soon as `universe` is initialized (early in bootstrap) rather than after the full multi-hour SEC bootstrap.
  - **Catch-up behavior (honest note):** orchestrator HF sync is a prelude opt-out that writes `sync_runs`, not `job_runs`; the boot catch-up (`_catch_up`, job_runs-based) therefore always treats it as never-run-successfully, so `catch_up_on_boot=True` fires it on *every* controller boot, not just after a genuinely missed window. This is intended under motivation (b) — fire at boot to populate the dashboard immediately. Cost is bounded (one portfolio + FX fetch); a restart loop re-fires it each boot but each fire is cheap and idempotent.
  - **Known residual (#1442):** the layer-init guard (`executor.py` `_layer_initialization_blocks`) is fail-open on a transient DB error in the init-check — it logs and lets the layer run. This carve-out *increases the reachability* of that path: pre-#1435 the universal gate blocked `portfolio_sync` during bootstrap entirely, whereas now it runs gated only by the layer-init check, so a transient DB failure at the check during bootstrap could let `portfolio_sync` run before `universe` is visible (the adapter then FK-fails, is caught, and retries). The fail-open behavior itself is pre-existing; fail-closing the init-check is tracked in #1442.
...
- `docs/specs/etl/coverage-model.md` §Layer 1/2/3.
- `docs/specs/etl/discovery-layers.md` (#1155 spec).
