...
- **Current volume**: Form 3 lifetime per issuer ~5-30 filings; Form 5 across the universe ~750-1900/year (45,845 since 1996 = 0.15-0.40 per US-filer per year).
- **Half-life**: **latest only matters** — Form 3 is the initial filing, supplanted by ongoing Form 4s; Form 5 is annual catch-up.
- **Consumers**: insider registry table (who is registered as an insider for this issuer); operator-visible baseline-only chart panel sourced from `list_baseline_only_insider_holdings`.
- **Ingest depth cap (PR10b SHIPPED)**:
  - **Form 3** — read-side latest-per-pair via `DISTINCT ON (filer_cik, security_title, is_derivative) ORDER BY as_of_date DESC, accession_number DESC, row_num ASC` in `list_baseline_only_insider_holdings` ([insider_form3_ingest.py:778-836](app/services/insider_form3_ingest.py#L778-L836)), AND the `NOT EXISTS` anti-join that excludes filers with non-tombstoned Form 4 activity. No write-side schema change: `insider_filings.accession_number` PK + per-accession append are the schema contract (3/A amendments append; the read picks the latest by tie-break). `INSIDER_FORM3_BACKFILL_FLOOR_YEARS = None` stays — a 10-year-old Form 3 for a still-serving officer IS the correct baseline. PR10b pins both the DISTINCT ON shape AND the anti-join via `scripts/check_form3_latest_per_pair.sh` (whitespace-normalised awk walker; resilient against reformatting, brittle only against semantic changes — Codex 1a HIGH lesson).
  - **Form 5** — ingest-time `filed_at` horizon helper at **18 months** (`INSIDER_FORM5_RETENTION_MONTHS = 18`, `form5_retention_cutoff()` / `form5_within_retention()` in [insider_transactions.py](app/services/insider_transactions.py)) wired into every writer chokepoint:
    1. **Manifest-worker pre-fetch gate** — `_parse_form5` ([manifest_parsers/insider_345.py:586](app/services/manifest_parsers/insider_345.py#L586)) tombstones pre-cap accessions with `error="retention floor"`. Pre-fetch placement means no `filing_raw_documents` row is written; recovery via `sec_rebuild` source-reset, not parser-version rewash.
    2. **Bulk dataset transactions loop** — `ingest_insider_dataset_archive` per-row gate at the NONDERIV_TRANS write loop ([sec_insider_dataset_ingest.py:338-340](app/services/sec_insider_dataset_ingest.py#L338-L340)). Per-form `if-elif` ladder so a single archive walk uses ONE retention-cutoff anchor per form (Form 4 + Form 5 cutoffs are computed at archive scope).
    3. **Bulk dataset holdings loop** — same gate at the NONDERIV_HOLDING write loop ([sec_insider_dataset_ingest.py:405-407](app/services/sec_insider_dataset_ingest.py#L405-L407)). Rare holdings-only Form 5 filings (NPV-style historical filings) must not slip past the cap on the second loop branch.
    4. **`sync_insiders` chokepoint** — `ownership_observations_sync.sync_insiders` ([ownership_observations_sync.py:124](app/services/ownership_observations_sync.py#L124)) is a steady-state observations writer that mirrors typed-table rows into the observations layer; a post-cap re-run would re-write pre-cap Form 5 observations without the gate. **LEFT JOIN** to `filing_events fe ON fe.provider_filing_id = it.accession_number AND fe.provider = 'sec'` (Codex 1b MED — INNER would silently shed Form 4 rows lacking a manifest entry). Form 5 branch filters `fe.filing_date IS NOT NULL AND fe.filing_date >= form5_cutoff`; Form 4 branch ignores `fe` entirely (Form 4 sync_insiders cap is a PR4 follow-up ticket, NOT closed here).
  - **Why 18m not 12m**: a 1y cap drops the only/latest Form 5 for a pair the moment it crosses the year boundary, leaving zero Form 5 visibility for weeks until the next annual lands. 18m gives the annual-cadence buffer (Codex 1a MED on PR10b plan). Worst case admits BOTH the prior-year Form 5 and the current-year Form 5 briefly — operator-visible impact is zero because the read side de-dupes via cumulative Form 4 rollup (`refresh_insiders_current` aggregates observations + Form 3 baseline; Form 5 transactions flow through the same cumulative path).
  - **Why ingest gate not latest-per-pair-at-write**: Form 5 transactions land in `insider_transactions` (PK `accession_number` + `txn_row_num`) just like Form 4 — latest-per-pair at write time is not enforceable without a schema overhaul. The 18m `filed_at` window is the closest annual-cadence approximation.
- **Chokepoint coverage (PR10b)**: four Form 5 write chokepoints (manifest pre-fetch + bulk transactions loop + bulk holdings loop + sync_insiders) all share the helper `form5_within_retention(filed_at)` / `form5_retention_cutoff()`. One Form 3 read chokepoint (`list_baseline_only_insider_holdings`). Lint guards `scripts/check_form5_retention.sh` (invariants A/B/C/D for Form 5) and `scripts/check_form3_latest_per_pair.sh` (invariants A/B/C/D for Form 3) wired into `.githooks/pre-push` after `check_business_summary_latest_only.sh`. Both lint guards use awk-based block parsing (PR4 Codex 1c lesson) and empty-grep `wc -l` guards (PR10a Codex iter 1 lesson).
- **No `_ingest_single_accession` for Form 5**: the per-filing path under `insider_transactions._process_candidates` filters `filing_type IN ('4', '4/A')` — Form 5 only enters via the three chokepoints above.
- **No rewash / no SQL repair sweep**: Form 5 shares Form 4's parser (`parse_form_5_xml`) but has no `_apply_form5` rescue function and no `sync_form5` SQL sweep. If a future PR adds either, that PR shares the helpers AND extends `check_form5_retention.sh`.
- **Cohort bound**: SEC universe filter (`instruments.country='US'` after PR1 + `is_tradable=TRUE`) constrains the candidate set. No insider-filer recency cohort (filer-CIK universe is walked per-issuer via `filing_events`, not a curated table).
- **Existing rows**: untouched. The pre-wipe (§6.3) + clean re-run will land bounded sets under both Form 5 cap and Form 3's latest-per-pair contract by construction.
- **Why this matters**: ingest budget — pre-cap Form 5 universe-wide × 33y = days of SEC bandwidth; the 18m cap shaves it to ≤2 filings per insider-pair. Form 3 latest-per-pair pin is the bug-class lockout — a future refactor that drops the anti-join or rearranges the DISTINCT ON columns would silently double-count filers on the ownership ring; the lint guard makes that a push-time failure.
...
- **`ownership_institutions_current` size oddity**: 2.8 GB is huge for a "current snapshot" — investigation needed. Either stores wide rows with embedded payload, or write-through is dumping more than current state. Separate audit ticket (PR12). **Resolved by PR12**: root cause was the DELETE+INSERT writer (`refresh_institutions_current` rewrote every row of an instrument's slice on each call → repair-sweep predicate `c.refreshed_at < MAX(obs.ingested_at)` re-fired forever once `now() < clock_timestamp()` race put `refreshed_at` behind the freshly-bumped `ingested_at`). PR12 rewrote the writer as PG17 `MERGE … WHEN NOT MATCHED BY SOURCE` with `IS DISTINCT FROM` diff predicate excluding `refreshed_at`, and moved the drift watermark to a new `ownership_refresh_state` side-table. Spec: `docs/_archive/2026-05-21-pr12-ownership-current-writer-merge.md`.
- **`refresh_institutions_current` is NOT a writer chokepoint** (for ingest-side caps): it derives `_current` from `_observations` via diff-aware MERGE (PR12 — was DELETE+INSERT pre-PR12). Spec §4.5 "always-current snapshot" + spec §6.3 "existing rows untouched" together mean the refresh path is exempt from the ingest-side retention cap — capping it would actively delete pre-wipe pre-cap rows from `_current` on every repair sweep, contradicting §6.3.
- **Existing rows**: untouched until pre-wipe (§6.3).
...
- **Existing rows**: untouched until pre-wipe (§6.3).
- **Why this matters**: combined 4.1 GB. Post-wipe + clean re-run under 8q + PR12 audit projects to ~0.5 GB (same proportional shape as §4.5). **PR12 SHIPPED**: `refresh_funds_current` migrated to diff-aware MERGE + watermark side-table; bloat root cause closed for the funds slice too (10.22% tuple density on `ownership_funds_current` pre-fix per Task 0 baseline).
...
- **Current volume**: ~575k SC 13D/G manifest rows already ingested via the legacy daily-index path (`app/services/filings_history.py` → `sec_daily_index_reconcile`). PR11 v8 attaches the retention cap + edgartools adapter to that existing stream; no PR11-owned discovery layer (see v8 empirical pivot note below).
...
- **Ingest depth cap (PR11 SHIPPED v8 2026-05-21)**: helper `blockholders_retention_cutoff() -> date` returning `max(today - 3y, 2024-12-18)` in `app/services/blockholders.py`. Floor catches up to 3y by 2027-12-18; until then mandate date wins. Pre-2024-12-18 filings unreachable by construction (SEC XBRL mandate effective date; HTML-only legacy filings not parseable by edgartools per skill_edgartools.md G11). 3 chokepoint gates + 1 exemption:
  - **B** manifest worker pre-fetch gate at `app/services/manifest_parsers/sec_13dg.py::_parse_13dg` — `blockholders_within_retention(row.filed_at)`; tombstone with `error="retention floor"` BEFORE `store_raw` + `fetch_document_text` (saves SEC HTTP budget)
  - **C** observations sync gate at `app/services/ownership_observations_sync.py::sync_blockholders` — `bf.filed_at >= cutoff` parameter (NOT `fe.filing_date` — Codex 1a HIGH null-reject lesson)
  - **F** rewash gate at `app/services/rewash_filings.py::_apply_blockholders` — branch-order: SELECT COUNT(*) → happy-path (existing rows) uncapped per §6.3; rescue-path (zero rows) applies retention helper
  - **D refresh_blockholders_current EXEMPT** per §6.3 (mirror of §4.5 13F-HR precedent)
- **Discovery architecture**: legacy daily-index path (`sec_daily_index_reconcile`) writes `sec_filing_manifest` rows (`subject_type='blockholder_filer'`, `cik=filer_cik`, `instrument_id=NULL` per CHECK constraint). PR11 is NOT discovery-side — chokepoint A from earlier drafts was retired in the v8 empirical pivot.
- **Parser**: `_parse_13dg` adopts `edgartools.beneficial_ownership.schedule13.Schedule13D.parse_xml` / `Schedule13G.parse_xml` via new `_schedule13_adapter` module (dict + nested-dataclass-attr access; field-name + Decimal-type conversion to repo's existing `BlockholderFiling` shape; downstream `_upsert_filing_row` + `_record_13dg_observation_for_filing` unchanged). CUSIP-only instrument resolution via `_resolve_cusip_to_instrument_id`; CUSIP-unresolved writes `blockholder_filings` rows with `instrument_id=NULL` + `blockholder_filings_ingest_log.status='partial'` (audit trail preserved per parser docstring; CUSIP-resolution gap tracked by #740).
- **Lint guard `scripts/check_13dg_retention.sh`**: 7 placement invariants A/C/D/E/F/G/H. Wired into `.githooks/pre-push`. Awk-based block walkers strip docstrings (Codex 1c lesson) so legitimate negative-example mentions in docstrings don't false-trip.
- **Existing rows**: untouched by PR11 ingest-side caps per §6.3. The pre-wipe + clean re-run is the only deletion event.
- **v8 empirical pivot 2026-05-21**: the universe-issuer-CIK discovery layer this section originally specified was RETIRED after operator smoke against AAPL/GME/MSFT/JPM/HD returned 0 efts.sec.gov hits — `efts.sec.gov/LATEST/search-index` post-2024-12-18 does NOT index SC 13D/G by SUBJECT CIK, only by FILER CIK. Discovery layer + hint table + 5-case CUSIP-vs-hint branch + bootstrap stage S27 + lint invariants B/I/J/K/L all dropped (sql/162 + sql/161 + parser simplification + scheduler rollback + lint prune). The legacy daily-index path remains the SC 13D/G discovery mechanism.
- **Closes**: #1249, #1250 (PR #1251 cleanup landed pre-PR11 KNOWN_FILING_AGENT_CIKS guards on the existing manifest parsers).
...
- **Ingest depth cap**: **latest N-CEN per CIK — PR9 SHIPPED.** Unlike PR5-PR8 (which add ingest-time horizon caps to observations tables), N-CEN enforces latest-only **structurally**, not via a horizon helper:
  ...
  2. **Writer** — `_upsert_classification` ([app/services/ncen_classifier.py:287](app/services/ncen_classifier.py#L287)) is the sole INSERT path; its UPSERT clause promotes in place when a newer N-CEN appears AND its row-constructor `WHERE (EXCLUDED.filed_at, EXCLUDED.accession_number) >= (existing.filed_at, existing.accession_number)` predicate refuses to demote to an older accession (Codex 1a HIGH + Codex 2 Medium on PR9 — makes the database the monotonicity oracle so a stale caller / one-off operator script cannot silently overwrite the newer classification, with same-day accession_number tie-break for the N-CEN/A-same-day-as-original case).
...
- **Chokepoint coverage (PR9)**: smallest of any PR in this rubric. No manifest-worker parser (`app/services/manifest_parsers/sec_n_cen.py` does not exist), no bulk-dataset path, no rewash function, no SQL repair sweep, no scheduled cron job — `classify_filers_via_ncen` is invoked only from the operator-driven `scripts/seed_holder_coverage.py:449`. The four structural invariants above ARE the cap. Lint guard `scripts/check_ncen_latest_only.sh` (four placement invariants A / B / C / D — C pins the no-demotion predicate AND same-day accession_number tie-break from Codex 1a HIGH + Codex 2 Medium; D additionally forbids `reversed(` / `sorted(` / `.sort(` inside `_find_latest_ncen` so newest-first iteration order is locked from Codex 2 Medium) pins the invariants against regression and is wired into `.githooks/pre-push`. Two existing tests already pin the runtime behaviour (`test_picks_latest_ncen` selects the newest of multiple N-CENs in one submissions array; `test_re_run_upserts_in_place` confirms re-runs against the same accession produce 1 row); PR9 adds three new tests: `test_promotes_to_newer_accession_across_passes` (pass 1 writes row(A); pass 2 with array showing A + newer B promotes to row(B)), `test_does_not_demote_to_older_accession` (pass 1 writes row(newer B); pass 2 showing only older A is REFUSED by predicate — row stays at B AND `fetched_at` unchanged, the strongest no-mutation proof since `classifications_written` only counts attempts), and `test_same_day_older_accession_does_not_clobber_newer` (both accessions filed same calendar day → identical `filed_at` value → tie-break on accession_number lexicographically refuses overwrite).
...
**Open question**: `ownership_institutions_current` is 2.8 GB — far larger than expected for "current snapshot". Either stores wide rows with embedded payload, or write-through dumps more than latest. **Separate audit ticket.** Not blocking this spec. **Resolved by PR12** — root cause was DELETE+INSERT writer + repair-sweep forever-loop, not wide-row payload. Writer rewritten to diff-aware MERGE; drift watermark moved to new `ownership_refresh_state` table. Existing-row reshape still happens via the §6.3 pre-wipe + clean re-run. See `docs/_archive/2026-05-21-pr12-ownership-current-writer-merge.md`.
...
- **PR3 — SHIPPED.** Filing events 10y rolling cap at the writer chokepoints (3 `INSERT INTO filing_events` sites across `app/services/filings.py` × 2 + `app/services/fundamentals.py` × 1). Helper `filing_within_retention(filing_date)` + constant `FILING_EVENTS_RETENTION_YEARS = 10` in `app/services/filings.py`. Lint guard `scripts/check_filing_events_retention.sh` pins the chokepoint surface (parity invariant: every `INSERT INTO filing_events` is preceded by a `filing_within_retention(...)` call-site; cross-cutting invariant: no INSERTs outside the 2 canonical files). Applied uniformly across every filing_type via every discovery writer (atom, daily-index, first-install drain, per-CIK poll, targeted rebuild, bulk dataset — all route through the gated upsert helpers). Schema columns + existing rows untouched.
...
- **PR10b — SHIPPED.** Form 3 / Form 5 latest-per-pair cap. **Form 3**: read-side latest-per-pair already wired in `list_baseline_only_insider_holdings`; PR10b adds `scripts/check_form3_latest_per_pair.sh` pinning four invariants — (A) function defined once, (B) DISTINCT ON columns + ORDER BY prefix match (exactly 2 occurrences of `iih.filer_cik, iih.security_title, iih.is_derivative` whitespace-normalised — SQL contract requires DISTINCT ON columns to lead ORDER BY), (C) three-tier tie-break (`as_of_date DESC, accession_number DESC, row_num ASC`), (D) `NOT EXISTS` anti-join against non-tombstoned Form 4 rows for the same filer (Codex 1a MED). Two new behaviour tests: multi-pair latest-per-pair + multi-pair with mixed Form 4 activity. **Form 5**: ingest-time helper `INSIDER_FORM5_RETENTION_MONTHS = 18` + `form5_retention_cutoff()` / `form5_within_retention()` at every writer chokepoint — manifest pre-fetch gate (`_parse_form5`), bulk dataset transactions loop, bulk dataset holdings loop, AND `sync_insiders` LEFT JOIN gate (Codex 1a HIGH — sync was the missing chokepoint; Codex 1b MED — LEFT not INNER, so Form 4 rows lacking manifest entry still sync). Width 18 months instead of 12 to provide annual-cadence buffer (Codex 1a MED — a 1y cap drops the only/latest Form 5 for weeks at year-boundary). PR4 Form 4 sync_insiders gap explicitly carved out and deferred to a follow-up ticket. Lint guard `scripts/check_form5_retention.sh` with four placement invariants A/B/C/D. Both lint guards wired into `.githooks/pre-push`.
- **PR11 — SHIPPED v8 empirical pivot 2026-05-21.** SC 13D/G `max(today - 3y, 2024-12-18)` cap floor (XML mandate alignment) attached to the existing legacy daily-index discovery stream (~575k SC 13D/G manifest rows already ingested pre-PR11). 3 chokepoint gates + 1 exemption: **B** manifest pre-fetch (`blockholders_within_retention(row.filed_at)` BEFORE `store_raw` + `fetch_document_text`) / **C** sync `bf.filed_at >= cutoff` (NOT `fe.filing_date` — Codex 1a HIGH null-reject lesson) / **F** rewash rescue-path (branch-order SELECT COUNT(*) → happy-path uncapped per §6.3 / rescue-path applies helper) + **D refresh_blockholders_current EXEMPT** per §6.3 (mirror of §4.5 13F-HR precedent). Edgartools `Schedule13D/G.parse_xml` adopted via `_schedule13_adapter` (dict + nested-dataclass-attr access → repo `BlockholderFiling`). `_parse_13dg` uses CUSIP-only resolution via `_resolve_cusip_to_instrument_id`; CUSIP-unresolved writes `instrument_id=NULL` + `status='partial'` (audit trail preserved; CUSIP-resolution gap tracked by #740). Lint guard `scripts/check_13dg_retention.sh` invariants A/C/D/E/F/G/H wired into `.githooks/pre-push`. Same PR retired dormant filer-seed path (4 entrypoints deleted from `blockholders.py` + `blockholder_filer_seeds` table dropped via sql/161 + `scripts/seed_holder_coverage.py` surgical edit + `tests/test_blockholders_ingester.py` dormant tests removed). **Empirical pivot history**: original PR11 design (Codex 1a-1g 7 rounds approved + plan Codex 1a/1b/1c/1d 4 rounds approved) shipped a universe-issuer-CIK discovery layer + `sec_13dg_discovery_issuer_hint` multi-row hint table + 5-case CUSIP-vs-hint cross-validation branch + bootstrap stage S27 + lint invariants B/I/J/K/L. Operator smoke against AAPL/GME/MSFT/JPM/HD on 2026-05-21 returned 0 efts.sec.gov hits — `efts.sec.gov/LATEST/search-index` post-2024-12-18 indexes SC 13D/G by FILER CIK only, not SUBJECT CIK. Discovery layer + hint table + 5-case branch + S27 + lint B/I/J/K/L all dropped (sql/162 + sql/161 + parser simplification + scheduler rollback + lint prune); stage-count assertion bumped 27 → back to 26. Closes #1249, #1250 (PR #1251 pre-cleanup landed agent-CIK guards).
- **PR12 — SHIPPED.** `ownership_*_current` writer rewrite (DELETE+INSERT → diff-aware PG17 `MERGE … WHEN NOT MATCHED BY SOURCE`) + new `ownership_refresh_state` watermark side-table separating drift detection from `_current.refreshed_at` (no more repair-sweep forever-loop). All 7 helpers migrated (insiders / institutions / blockholders / treasury / def14a / funds / esop); `_drifted_instruments` predicate switched to obs-anchored CTE aggregate; `_CATEGORIES` expanded 5 → 7; PG ≥ 17 boot guard at lifespan startup; 93-clause lint guard wired into `.githooks/pre-push`; 52 parametrised tests; orphan-audit script. Existing-row reshape still happens via the §6.3 pre-wipe + clean re-run. Implementation spec: `docs/_archive/2026-05-21-pr12-ownership-current-writer-merge.md`. Closes the §4.5 / §4.6 / §6.4 "ownership_*_current size oddity" question.
...
2. **DB size**: post-clean-rerun `pg_database_size('ebull')` measured (excludes WAL — Postgres WAL lives outside per-database size accounting; tracked separately at the filesystem level via `pg_stat_wal` / `pg_ls_waldir()` if needed). Provisional breakdown:
   ...
   - Operator acceptance tiers (all measured AFTER the §6.3 pre-wipe + clean re-run; the *tier* selected for acceptance depends on what state PR12 reached at wipe time):
     - **v1 bar — `<20 GB hot`**: PR12 audit complete + caps merged but remediation partial / deferred (current snapshots remain near 5.3 GB combined under whatever the audit identifies as the cause). ~57% reduction from current 43 GB. Acceptable for v1 ship. (Pre-PR12 tier.)
     - **Phase 2 stretch — `<15 GB hot`**: PR12 fully solved (current snapshots ≤ 1 GB combined). ~65% reduction. **PR12 SHIPPED — writer fix landed; existing rows reshape via §6.3 pre-wipe + clean re-run measures the actual tier.**
...
- **`ownership_*_current` size oddity (PR12)**: ~~biggest unknown affecting §8 DB-size target~~ — **resolved by PR12 writer rewrite (SHIPPED)**. Three-tier acceptance retained for the §6.3 pre-wipe + clean re-run measurement: v1 bar `<20 GB`, Phase 2 stretch `<15 GB` (PR12 solved), ambitious `<10 GB` (PR12 + companyfacts further tightening — Phase 3 if needed). Spec: `docs/_archive/2026-05-21-pr12-ownership-current-writer-merge.md`.
...
- Implementation plan = separate doc (`docs/_archive/2026-05-19-data-retention-impl.md`), Codex 1a/1b on that.
...
- PR1 (#1238), PR2 (#1237), PR3 (#1239), PR4 (#1240), PR5 NUMERIC fix
  ...
  cap (#1244), PR9 N-CEN latest-only invariant pin (#1245),
  PR10a business-summary latest-only invariant pin (#1246), and
  **PR10b Form 3 / Form 5 latest-per-pair cap (this commit)** all
  shipped.
- PR11 (13D/G activate dormant pipeline w/ 3y historical cap), PR12
  (`ownership_*_current` audit) remain — both are ingest-side cap
  only, no row deletion. The pre-wipe is the single operator event
  at the end.

Open PR4 follow-up surfaced by PR10b Codex 1a:

- `sync_insiders` does NOT gate Form 4 by the 3-year retention cap.
  PR10b explicitly carves out Form 4 in the new sync WHERE clause
  (Form 4 branch ignores `fe.filing_date`) and files a follow-up
  ticket so the gap is tracked. Rationale for not closing it inside
  PR10b: scope creep into PR4's territory; the deferred ticket is
  the honest record. Closing it requires the same shape used for
  Form 5 sync gate — extend the LEFT JOIN predicate to also gate
  the Form 4 branch on `fe.filing_date >= form4_cutoff` and add
  the corresponding lint anchor + regression test.
