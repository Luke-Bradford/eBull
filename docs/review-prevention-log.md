...
- Prevention: when extending a test seeder to mirror legacy → observations write-through, copy every `WHERE` / `IF` guard that the production ingest applies before its `record_*_observation` / `refresh_*_current` calls. Specifically: `is_derivative = FALSE` for insider Form 4, `is_put_call IS NULL` (vs `EQUITY` exposure_kind) for 13F equity-only filtering, `aggregate_amount_owned IS NOT NULL` for blockholders, `shares IS NOT NULL` for DEF 14A. Self-review prompt: side-by-side compare the seeder's mirror block against the production ingester body it claims to mimic, and assert the same row-skip predicates apply. Easy to grep for: every `record_*_observation` call site in `tests/` should have an immediately-preceding production-equivalent guard.
