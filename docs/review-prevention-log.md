...
### Changing a component prop default with context-dependent semantics

- First seen in: #652 (#650/#647 PriceChart polish).
- Symptom: `ChartCanvas`'s `containerClassName` default was changed from `"h-[340px] w-full"` to a flex-dependent `"h-full min-h-[340px] w-full flex-1"`. The new default works only when the parent is a flex/grid container with a definite height; in any block-layout caller `h-full` resolves to 0 and `flex-1` is ignored, collapsing the canvas. Today only one caller exists and it now passes the flex variant explicitly, but a future caller that omits the prop would silently regress.
- Prevention: when a component prop default has parent-context dependencies, keep the default the safe one and require callers that need the context-dependent variant to pass explicitly. At self-review for any prop default change: grep for every `<Component` usage that omits the prop and confirm each is in a parent context where the new default still makes sense.
- Enforced in: PriceChart.tsx default reverted to `"h-[340px] w-full"`; PriceChart's wrapper passes the flex variant explicitly with an inline comment that warns future callers.
