...
- Enforced in: this prevention log; tracked under issue #1367. Hookify rule for UserPromptSubmit / Stop detection of these tokens is a follow-up sub-task — when scaffolded, link the rule path here. Memory cross-link: `[[feedback_post_push_cycle]]` + `[[feedback_no_fake_polling]]` reinforced with the explicit "any wait, not just post-push" extension.
