...
- Enforced in: this prevention log; `.claude/skills/data-engineer/SKILL.md` ("One beneficial owner, counted once — MAX overlapping, SUM additive"); `app/services/ownership_rollup.py::_reconcile_owner_once` + `_source_rows_and_total` (`_ADDITIVE_SOURCES` = `{form4, form3}` only); `tests/test_ownership_owner_reconcile.py` (`test_def14a_beneficial_and_voting_not_summed`, `test_13f_managed_assets_never_added_to_insider`, `test_source_rows_and_total_helper`).
