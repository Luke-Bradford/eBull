...
- Enforced in: this prevention log; `frontend/scripts/check-chart-integrity.mjs` (bans the raw palettes outside `chartTheme.ts` + the cubic curve type, scoped to `components/fundamentals/`); `frontend/package.json` (`charts:check`); `.githooks/pre-push`; `.claude/skills/frontend/design-system.md` (Chart theming + "Chart series must not draw unreported values").
