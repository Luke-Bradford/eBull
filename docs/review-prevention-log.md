...
- Limitation: source-registry coverage prevents the `KeyError` crash but does NOT make these jobs actually run. The orchestrator's outer `JobLock(orchestrator_*_sync, source='db')` holds the `db` source-lock for the entire orchestrator run, so any inner adapter mapped to `db` self-skips with `JobAlreadyRunning` → `PREREQ_SKIP`. Tracked separately in #1184 (drop redundant inner `_run_with_lock` JobLock, or introduce per-job Lane variants).
