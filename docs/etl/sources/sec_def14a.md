...
**Form scope (`_FORM_TO_SOURCE`).** `DEF 14A` (definitive) + `DEFA14A` (additional definitive) + `DEFM14A` (merger) + `DEFR14A` (revised definitive) map to `sec_def14a`. **`PRE 14A` / `PRER14A` are deliberately excluded from `sec_def14a` (#1320)** — preliminary proxies are pre-finalisation drafts; the definitive DEF 14A that follows is what the ownership pipeline ingests. Mapping PRE 14A into `sec_def14a` routed 6k+ drafts into this manifest namespace which the parser tombstoned pre-fetch (§7) — wasted worker cycles + polluted `WHERE source='sec_def14a'` reads. The parser's PRE-14A tombstone branch stays as defense-in-depth for any PRE row that reaches the worker via a legacy/manual seed. Existing mis-seeded rows purged by `sql/182_purge_pre14a_manifest_rows.sql`. As of #1892, `PRE 14A` / `PRER14A` map instead to the wholly separate `sec_pre14a` proposal-signal source (`docs/etl/sources/sec_pre14a.md`) — `map_form_to_source('PRE 14A') == 'sec_pre14a'`, never `'sec_def14a'`.
