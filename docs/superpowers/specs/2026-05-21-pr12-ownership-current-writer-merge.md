...
**Treasury MERGE ON-clause caveat (PG MERGE runtime discovery, 2026-05-21 during impl)**: PG MERGE compiles to an internal `FULL OUTER JOIN` between target and source; FULL JOIN requires at least one **equi-joinable** (merge- or hash-joinable) condition between the two sides. The other 6 helpers satisfy this automatically because their multi-col PK gives a `tgt.<pk_col_n> = src.<pk_col_n>` equi-join alongside the const `tgt.instrument_id = %(iid)s` clamp. Treasury's single-col PK `(instrument_id)` has no second equi-join column — using `ON tgt.instrument_id = %(iid)s` alone errors at execute time with `FeatureNotSupported: FULL JOIN is only supported with merge-joinable or hash-joinable join conditions`. Treasury MUST use `ON tgt.instrument_id = src.instrument_id`; the USING subquery already filters `WHERE instrument_id = %(iid)s` so 1×1 cardinality is preserved and the const clamp is semantically equivalent. **Defence-in-depth**: the `WHEN NOT MATCHED BY SOURCE AND tgt.instrument_id = %(iid)s THEN DELETE` clause STILL carries the literal const clamp — only the ON-clause shape differs. Lint **D1 expected count for treasury = 0** (no const literal in ON); **D2 expected count for treasury = 1** (const literal in DELETE clause). All 6 other helpers: D1 = 1, D2 = 1.
