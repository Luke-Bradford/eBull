...
- **Current volume**: backfilled by PR11 via universe-issuer-CIK-driven discovery (`efts.sec.gov/LATEST/search-index`) at `max(today - 3y, 2024-12-18)` floor. Pre-2024-12-18 filings unreachable by construction (SEC XBRL mandate effective date; HTML-only legacy filings not parseable by edgartools per skill_edgartools.md G11). PR11 also retired the dormant filer-seed code path (4 entrypoints + `blockholder_filer_seeds` table dropped via sql/161).
...
- **Ingest depth cap (PR11 SHIPPED)**: helper `blockholders_retention_cutoff() -> date` returning `max(today - 3y, 2024-12-18)` in `app/services/blockholders.py`. Floor catches up to 3y by 2027-12-18; until then mandate date wins. 4 chokepoints:
  - **A** discovery query at `app/services/sec_13dg_discovery.py::_resolve_discovery_startdt` — watermark = `max(floor, MAX(blockholder_filings.filed_at) WHERE issuer_cik = ? - 7d)`; bootstrap mode always returns floor
...
- **Discovery architecture**: per-issuer-CIK walk against `efts.sec.gov/LATEST/search-index?ciks={issuer}&forms=SC+13D,SC+13D/A,SC+13G,SC+13G/A&dateRange=custom&startdt=...&enddt=...`. Writes `sec_filing_manifest` rows (`subject_type='blockholder_filer'`, `cik=first non-issuer non-agent CIK from ciks[]`, `instrument_id=NULL` per CHECK constraint) + multi-row `sec_13dg_discovery_issuer_hint (accession_number, instrument_id) PK` side-table (handles GOOG/GOOGL, BRK.A/BRK.B share-class siblings on shared CIK per #788) in ONE `conn.transaction()` block (atomicity lint K).
- **Parser**: `_parse_13dg` adopts `edgartools.beneficial_ownership.schedule13.Schedule13D.parse_xml` / `Schedule13G.parse_xml` via new `_schedule13_adapter` module (dict + nested-dataclass-attr access; field-name + Decimal-type conversion to repo's existing `BlockholderFiling` shape; downstream `_upsert_filing_row` + `_record_13dg_observation_for_filing` unchanged). 5-case CUSIP-vs-hint cross-validation branch (CASE A CUSIP-in-hints / CASE B single-hint fallback / CASE C ambiguous → NULL + log / CASE D-in-universe / CASE D-out-of-universe → NULL + log / CASE E legacy daily-index path).
- **Lint guard `scripts/check_13dg_retention.sh`**: 12 placement invariants A-L. Wired into `.githooks/pre-push`. Awk-based block walkers strip docstrings (Codex 1c lesson) so legitimate negative-example mentions in docstrings don't false-trip.
- **Bootstrap stage**: S27 `sec_blockholders_discovery` (lane=sec_rate, params={"mode": "bootstrap"}) added at `app/services/bootstrap_orchestrator.py`; stage-count assertion bumped 26 → 27; 11 lockstep updates (frontend stage list / runbook / tests / capability requires map / JOB_INTERNAL_KEYS).
...
- **PR11 — SHIPPED.** SC 13D/G activation via universe-issuer-CIK-driven discovery (`efts.sec.gov/LATEST/search-index`) at `max(today - 3y, 2024-12-18)` cap floor (XML mandate alignment). 4 chokepoints (A discovery query / B manifest pre-fetch / C sync `bf.filed_at` / F rewash rescue-path) + refresh-current EXEMPT per §6.3. Multi-row `sec_13dg_discovery_issuer_hint` side-table handles share-class siblings on shared CIK. Edgartools `Schedule13D/G.parse_xml` adopted via `_schedule13_adapter` (dict → repo `BlockholderFiling`). 5-case CUSIP-vs-hint cross-validation branch in `_parse_13dg` (CASE A in-hints / B single-hint fallback / C+D-out NULL+log / D-in discrepancy log / E legacy). Bootstrap stage S27 wired (`sec_blockholders_discovery`, lane=sec_rate, params={"mode":"bootstrap"}). Lint guard `scripts/check_13dg_retention.sh` invariants A-L wired into `.githooks/pre-push`. Same PR retired dormant filer-seed path (4 entrypoints deleted from `blockholders.py` + `blockholder_filer_seeds` table dropped via sql/161 + `scripts/seed_holder_coverage.py` surgical edit + `tests/test_blockholders_ingester.py` dormant tests removed). Spec Codex 1a-1g (7 rounds) + plan Codex 1a/1b/1c/1d (4 rounds) all green before dispatch. Closes #1249, #1250 (PR #1251 pre-cleanup landed agent-CIK guards).
