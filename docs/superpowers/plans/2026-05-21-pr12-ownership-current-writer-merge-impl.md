...
| `refresh_treasury_current` | DISTINCT ON `(instrument_id)`. **Extra WHERE filter** `AND treasury_shares IS NOT NULL`. Business cols `(ownership_nature, source, source_document_id, source_accession, source_url, filed_at, period_start, period_end, treasury_shares)`. Category `'treasury'`. **ON clause: `ON tgt.instrument_id = src.instrument_id`** — NOT the literal `tgt.instrument_id = %(iid)s` (Task 5 runtime discovery 2026-05-21 — PG MERGE compiles to internal FULL OUTER JOIN which errors `FeatureNotSupported: FULL JOIN is only supported with merge-joinable or hash-joinable join conditions` when the ON clause is only a const predicate; the other 6 helpers have a 2nd PK col providing the equi-join). USING subquery filters `WHERE instrument_id = %(iid)s` so 1×1 cardinality holds. The `WHEN NOT MATCHED BY SOURCE AND tgt.instrument_id = %(iid)s THEN DELETE` clamp is preserved (defence-in-depth). Lint D1 expected count for treasury = 0 (no const in ON); D2 expected count = 1 (const in DELETE). Spec §4.1 was amended in the same commit with the full caveat. |
...
3. For invariant D, awk-extract the ON-clause span (between `MERGE INTO ownership_${helper}_current AS tgt` and the first `WHEN`) and the WHEN NOT MATCHED BY SOURCE clause separately. D1 (ON span) expected counts: **0 for treasury** (PG MERGE FULL JOIN requires equi-join; treasury single-col PK uses `tgt.instrument_id = src.instrument_id` instead — Task 5 runtime discovery; spec §4.1 amended); **1 for the other 6 helpers**. D2 (NOT MATCHED BY SOURCE span) expected count: **1 for all 7 helpers** (the const clamp is preserved as defence-in-depth).
