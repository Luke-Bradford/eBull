...
1. `app/jobs/sources.py`: add `"sec_manifest"` to `Lane` + `_ALLOWED_SOURCES`; correct the stale "rate bucket" docstrings — `sources.py:84-86` ("every per-CIK fetch competes here") and `app/jobs/locks.py:14-25` module docstring — so the next reader doesn't re-add the mutex believing it's the rate gate. Mirror `sec_manifest` into `api/bootstrap.py:69 LaneApi` (+ `bootstrap_state.py::Lane` if it enumerates non-bootstrap lanes — verify at impl).
