...
### Where it runs
New scheduled job `jobs_liveness_watchdog` in the jobs process (`app/workers/scheduler.py` + body), cadence `every_n_minutes(interval=15)` (cheap: 2 aggregate queries + pure compute). Registered with `exempt_from_universal_bootstrap_gate=True` (a watchdog must run regardless of bootstrap state) and listed in the bootstrap-gating drift test's `NON_GATED_SCHEDULED` accordingly (#1504 pattern). Add to `_INVOKERS` for manual trigger. **Caveat (in-scope-but-honest):** because it is itself a scheduled job, it cannot report its own stall — that is the heartbeat path's job (see Scope).
