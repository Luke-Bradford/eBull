...
- **S15 `filings_history_seed`** (gap memo §2 #2) — **DEFERRED 2026-05-29**. The "master.idx 730d backfill" framing targets the wrong table (master.idx writes `sec_filing_manifest`; S15's consumers read `filing_events`), and a naive per-issuer `filing_events` skip-gate is unsound (many-writer provenance → false-positive skips that tombstone DEF 14A + zero insider observations). S8+S13 already bulk-seed issuer `filing_events`; S15 is the legacy backstop. The only sound optimisation gates on S13's completion signal (~0.5-1 day, correctness-sensitive) — not worth the ~6 min ROI. Full analysis: [`docs/proposals/etl/1337-s15-bulk-seed-skip.md`](1337-s15-bulk-seed-skip.md).
