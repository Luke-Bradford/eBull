...
| ~~`sec_8k_events_ingest`~~ | ~~SEC 8-K events ingest~~ | **on-demand** post-#1155 | — | `ingest_8k_events(conn, provider)` | bounded 200/run; manifest worker + `eight_k.py` (#1126) carry steady-state. Bootstrap stage 20 + sweep-adapter `sec_8k_sweep` + Admin Run-now kept. |
