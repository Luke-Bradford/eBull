...
def main() -> int:
    ...
    try:
        for name, edits, tests, selector in PROBES:
            ...
            # and the second reads as CAUGHT (prevention log, #2214). ⚠ Run
            # BEFORE the write loop below, not merely before `run`: this probe
            # mutates TWO files, so a baseline taken after a partial write would
            # be measuring one of them already broken.
