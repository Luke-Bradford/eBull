...
#  C. UPSERT clause present (with no-demotion predicate).
...
#         (Codex 1a HIGH on PR9). Without it the structural cap
#         enforces "at most one row" but not "latest one row" — a
...
if [[ ! -f "$FILE_CLASSIFIER" ]]; then
  ...
  # No-demotion predicate (Codex 1a HIGH on PR9). The exact literal is
  # unique enough that a single grep is unambiguous — the table name
  # would not appear in unrelated prose.
  demotion_guard_count=$(count_literal "$FILE_CLASSIFIER" "WHERE EXCLUDED.filed_at >= ncen_filer_classifications.filed_at")
  if (( demotion_guard_count != 1 )); then
    fail "$FILE_CLASSIFIER: expected exactly 1 no-demotion predicate 'WHERE EXCLUDED.filed_at >= ncen_filer_classifications.filed_at', found ${demotion_guard_count}."
  fi
fi
