...
PROBE_VERSION: Final = "market-sources-probe-v1"
...
def cadence(dates: Iterable[date]) -> dict[str, Any]:
    """Rows per year, weekday mix and the day-gap histogram of a sorted, de-duplicated date list."""
    days = sorted(set(dates))
    gaps = Counter((b - a).days for a, b in zip(days, days[1:], strict=False))
    return {
        "n": len(days),
...
def bisect_first_vintage(exists: Callable[[date], bool], lo: date, hi: date) -> date | None:
    """Earliest date in ``[lo, hi]`` for which ``exists`` holds, given it is monotone (False..True)."""
    if not exists(hi):
        return None
    if exists(lo):
        return lo
...
def count_revisions(old: dict[date, str], new: dict[date, str], cut: date) -> dict[str, int]:
    """Observations up to ``cut`` present in both vintages, and how many changed value."""
    common = [d for d in old if d <= cut and d in new]
    changed = sum(1 for d in common if _num(old[d]) != _num(new[d]))
    return {"common_observations": len(common), "changed": changed, "only_in_old": sum(d not in new for d in old)}
...
def _fred(client: _Client) -> dict[str, Any]:
    ...
    for series in FRED_SERIES:
        ...
        first_vintage = bisect_first_vintage(lambda d: alfred(d).status_code == 200, ALFRED_FLOOR, today)
        entry: dict[str, Any] = {
            "current": {
...
def _cot_dates(path: Path) -> tuple[set[date], set[date], str]:
    """All report 'as of' dates in a COT zip, those for the E-mini S&P 500 code, and the date column."""
    every: set[date] = set()
    emini: set[date] = set()
    column = ""
...
def _cot(client: _Client) -> dict[str, Any]:
    ...
    for family in ("legacy_futures", "tff_futures"):
        ...
        for href in links[family]:
            ...
            dates, sp, column = _cot_dates(path)
            every |= dates
            emini |= sp
            files.append({"href": href, "sha256": _sha256(path), "date_column": column, "report_dates": len(dates)})
        eras = {}
        ...
        out[family] = {
            "files": files,
            "all_markets": cadence(every),
            "eras": eras,
            "emini_sp_500": cadence(emini),
        }
    return out
...
def _wikipedia(client: _Client, symbols_by_stratum: dict[str, set[str]]) -> dict[str, Any]:
    ...
    for name, qid in WIKIDATA_US_EXCHANGES.items():
        rows = _sparql(
            ...
            f"WHERE {{ ?item p:P414 ?st . ?st ps:P414 wd:{qid} ; pq:P249 ?ticker . "
        ...
        exchanges[name] = {
            "qid": qid,
            "ticker_statements": len(rows),
            "items": len({r["item"] for r in rows}),
            "with_start_time": sum("start" in r for r in rows),
            "with_end_time": sum("end" in r for r in rows),
