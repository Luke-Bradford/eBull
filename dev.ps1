...
if ($elapsed -ge $timeout) {
    Write-Host "Postgres did not become ready in $timeout seconds - check: docker logs trader-os-postgres" -ForegroundColor Red
    exit 1
}
...
Write-Host "  /health/db - DB and migration status" -ForegroundColor Gray
