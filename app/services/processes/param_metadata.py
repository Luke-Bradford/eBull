    ...
    "sec_rebuild": (
        ...
        ParamMetadata(
            ...
            help_text=(
                ...
                "Note: a source with no data_freshness_index rows now "
                "ERRORS rather than reporting success with zero triples "
                "(#2379) — measured 2026-09-14, sec_xbrl_facts is the "
                "only enum value in that state (finra_short_interest "
                "has 1 row). A source can still resolve triples that "
                "the manifest worker then debug-skips (no parser "
