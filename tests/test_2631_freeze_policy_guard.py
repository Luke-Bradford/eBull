...
def test_every_freeze_script_calls_the_guard() -> None:
    ...
    # for #2837's S-E declaration (the fourth), #2840's S-H arm 1 (the fifth)
    # and #2901's quality arm (the sixth); each calls the guard and freezes
    # through the ledger like the other three.
    assert len(scripts) == 6, f"a new freeze script appeared: {[p.name for p in scripts]}"
    for path in scripts:
