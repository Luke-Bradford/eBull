...
It is not transactional. ``background_write_connection`` defaults to
``autocommit=True`` (``app/db/background_write.py:70``), so the INSERT is already
committed before the UPDATE is attempted and a failed statement leaves the
connection IDLE rather than aborted.
...
def test_a_skip_row_survives_an_arm_that_raises(
    ...
    monkeypatch.setattr("app.jobs.runtime.lost_fire_rearm_delay_seconds", _boom)
