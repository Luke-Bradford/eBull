...
class TestSystemJobs:
    ...
    def test_next_run_time_source_is_always_declared(self) -> None:
        """Since #719 the API no longer hosts APScheduler, so every
        next_run_time row is sourced from ``compute_next_run(now)``.
        The discriminator stays in the response shape for frontend
        compat but is permanently ``"declared"``.
        """
        _override_conn(_mock_conn())
        with patch(
            "app.api.system.check_job_health",
