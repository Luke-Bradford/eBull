...
- ⚠⚠ **`price_daily` is populated for 41% of the universe and FRESH for 11% (#2246, measured 2026-08-04).** `daily_candle_refresh` (`app/workers/scheduler.py:2474`) refreshes held ∪ T1/T2 ∪ `BENCHMARK_SYMBOLS` ∪ ≤200/night from `_T3_BOOTSTRAP_SELECT` — and that T3 branch carries `NOT EXISTS (price_daily)` (`:2458`), making it **seed-only**: a Tier-3 instrument leaves refresh scope on its first bar and nothing takes over unless it promotes (`total_score >= 0.55`) or is held/benchmark. Result: 3,523 of 3,838 priced T3 have no bar in 30 days, and every crypto / FX / commodity series is ~2 months stale (**#2254**, open). Two consequences for any new consumer: (1) **never assume `EXISTS(price_daily)` means usable** — check `max(price_date)`; (2) **coverage tier is implicitly US-only**, because it is driven by `fundamentals_snapshot`, which is SEC-fed, so all 1,383 T1/T2 instruments are `us_equity` and no non-US instrument has ever reached T1/T2. Scoping a market-wide surface on "scoring-eligible" therefore silently means "US filer". eToro *does* serve daily candles for the international set (27/28 probed, current to the prior close) — the 7,463 unpriced instruments are our selection gate, not provider supply, bar an irreducible 27 that return 0 bars.
