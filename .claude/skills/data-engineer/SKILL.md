...
| I6 | Soft-delete via tombstones, never hard-delete observations. **A re-parse supersedes a filing's prior rows** (`known_to = NOW()` scoped to `(instrument_id, source_document_id)` — per-instrument, because writers fan out over share-class siblings) and the writer's `ON CONFLICT DO UPDATE` sets `known_to = NULL` so rows the new parse still emits revive in the same tx. Without BOTH halves a parser fix is additive, not corrective: the conflict key is derived from the parsed value (`holder_name_key = lower(trim(holder_name))`), so a corrected row lands under a NEW key and the broken one stays live — and `refresh_*_current`'s `NOT MATCHED BY SOURCE` prune reads that same polluted set (#2140) | `ownership_*_observations.known_to`, `*_ingest_log` rows, manifest `tombstoned` state; `def14a_ingest.py::_record_def14a_observations_for_filing` + `ownership_observations.py::record_def14a_observation` (#2140). ⚠ `rewash_filings.py` 13F path still hard-DELETEs (#953) because its writer lacks the revive clause |
...
| I14 | One beneficial owner counted ONCE in the rollup, classified by most-specific role (insider > institution > blockholder). Aggregation: SUM only *additive* natures (Form 4 `direct`+`indirect`); MAX *overlapping* restatements (DEF 14A beneficial/voting, 13D/G vs 13F vs Form 4 — same shares, different lens). The additive SUM spans the additive SOURCES: a person's `direct` (Form 4) + `indirect` (Form 3) SUM across the two forms — `_merge_section16_forms` pools `{form4,form3}` into one channel before the cross-source MAX so the smaller form's lot isn't dropped (#1941). **Display:** "counted once" is honoured in the total AND the DISPLAY — `_build_slice` collapses an insider's additive lots into ONE line at the summed shares (Item 403 one-owner-one-line; per-lot direct/indirect split moves to the `lots` drilldown), so the L1 list shows one row per owner not one per lot (#1942). Collapse is **insiders-only** — `funds`/`def14a_unmatched` intentionally share a filer CIK across distinct rows, so a CIK-keyed collapse would wrongly merge them | `app/services/ownership_rollup.py::_reconcile_owner_once` + `_source_rows_and_total` + `_merge_section16_forms` (`_ADDITIVE_SOURCES`={form4,form3}) + `_collapse_owner_lots` (#1942, insiders-gated); specs `docs/specs/etl/2026-06-15-ownership-owner-once-dedup.md` (#1640) + `2026-07-04-ownership-collapse-owner-lots.md` (#1942); prevention-log "Same owner across reporting channels…" + "An identity-keyed collapse/dedup at a shared chokepoint must be scoped" |
...
| I16 | An institutional manager **family** (Vanguard, BlackRock, …) is ONE owner, counted once at `MAX(Σ family 13F holdings, family DEF 14A proxy, family 13G)`. The DEF 14A 5%-holder figure is the family-consolidated 13G (Reg S-K Item 403) — NOT a single 13F CIK and NOT independent ownership. MAX-once fixes BOTH the proxy double-count (Vanguard: proxy folds under the 13F sum) AND the family undercount (BlackRock: proxy fills the gap a missing sub-book CIK leaves) without ingesting the missing CIK. Family identity = curated in-code registry (CIK ∪ name-pattern, fail-closed). Read-path reject (never cap) any proxy/channel row > shares_outstanding (parser garbage). Pre-pass runs BEFORE `_reconcile_owner_once` (channel figures intact). Collapsed family keeps its 13F sub-CIK breakdown via `Holder.family_members` (display-only, non-additive). | `app/services/institutional_families.py` (registry); `app/services/ownership_rollup.py::_reconcile_institutional_families` + `_value_is_sane` + `FamilyMember`; spec `docs/specs/etl/2026-06-15-institutional-family-identity.md` (#1644 + #1649); prevention-log "Proxy 5%-holder is a family-consolidated restatement, not an independent additive wedge" |
| I17 | A Schedule 13D/G **group** (Rule 13d-5(b)) is ONE owner in the blockholders pie wedge, counted once at **MAX**. Members each report the IDENTICAL aggregate group stake on separate accessions/CIKs (each is deemed to beneficially own the whole group's holdings), so per-CIK dedup keeps all N and they sum N× (TKO: 3× ~94M = 282M vs ~94M real). **Never depend on `<memberOfGroup>`** — the element is empirically ABSENT from modern 13D XML; the legacy `blockholder_filings.member_of_group` column is noise ('a'/'b' both appear for real groups). Infer from same issuer + same `period_end` + near-equal non-round `aggregate_amount_owned` across DISTINCT non-null CIKs, with: member-count tiered tolerance (≥3 distinct CIKs → 0.1% band — a 3-way coincidence is negligible and catches personal slivers; exactly 2 → tight 0.001% gate — the coincidence-prone case); a round value (multiple of 100k) never seeds OR joins a cluster (round-lot coincidence); survivor-overlap exclusion (a CIK owner-once reconciles cross-channel is left untouched, else its other-channel rows orphan and re-count the block); max-anchored greedy clustering (no transitive chaining). Pure read-path pre-pass BEFORE `_reconcile_owner_once`; losers → `dropped_sources` + a `blockholder_group_collapse` `corrections_applied[]` entry. The Form 4 cross-channel explosion on all-insider control groups (insiders >100% of float) is a SEPARATE, larger bug → #1652. | `app/services/ownership_rollup.py::_reconcile_13d_groups` + `_is_collapsible_cluster` + `_is_group_block`; spec `docs/specs/etl/2026-06-16-blockholder-13d-group-collapse.md` (#1645); prevention-log "Verify the assumed group signal exists in the source before designing the pipeline" |
| I18 | A multi-class issuer (GOOG/GOOGL, HEI/HEI.A) whose classes share one SEC CIK has its rollup denominator set to the **combined all-class** share count, so every percentage is a combined-basis **lower bound** — the per-class `dei:EntityCommonStockSharesOutstanding` is dimensional (`StatementClassOfStockAxis` member) and **stripped by the companyfacts API** (sec-edgar §7.17), so `financial_facts_raw` holds only the combined us-gaap `CommonStockSharesOutstanding`. Numerators stay per-class-correct (CUSIP-resolved channels). Honest-degradation ONLY (no fabricated split): surface a `dual_class_denominator` caveat (server-owned `note`, FE callout, `__dual_class_denominator__` CSV memo row) — NOT a `CorrectionApplied` (removes no shares). Detect with THREE gates, all required (empirically zero false positives — fires on GOOG/GOOGL, HEI/HEI.A, METC/METCB): (1) denominator concept == `CommonStockSharesOutstanding` (us-gaap fallback = the multi-class fingerprint; a `dei` denominator means a single non-dimensional cover value → single-class issuer, ETF/ETN trust, or `.US` dual-listing → no understatement); (2) ≥2 instruments share the canonical CIK; (3) ≥2 distinct CUSIPs among the siblings (genuine separate securities, not a same-security `.US` listing dup). Self-heals as CUSIP coverage grows (BF.A/BF-B fires once its 2nd CUSIP lands). Limitation: a multi-class issuer with only ONE class in the universe (BRK.B alone) can't be caught by the sibling test → stays silently understated until #1590. Precise per-class denominator rides on **#1590 (DERA FSDS `num.tsv` `segments`)**. | `app/services/ownership_rollup.py::_detect_dual_class_denominator` + `DualClassDenominator` + `_dual_class_note` (#1646); sec-edgar §7.17; FAQ Q16 |
| I19 | The rollup **denominator** is independently cross-checked (`denominator_cross_check`, #1647 pt5) — it is the highest-leverage figure (every wedge ÷ it) AND the one figure the as-of (pt1) + sanity (pt4) layers cannot validate: a wrong-but-self-consistent denominator passes every internal check (#1646 was 2× wrong with perfect provenance). Single-class → tie-out `dei:EntityCommonStockSharesOutstanding` (cover-page) vs `us-gaap:CommonStockSharesOutstanding` (balance-sheet), NEAREST-period (not latest), `status` agrees(≤2%)/minor_skew(2–5%)/diverges(>5%). Dual-class → Σ sibling per-class FSDS vs combined all-class at the same FSDS instant, a `plausible`/`diverges` **subset bound** (untraded Class B is unverifiable → NOT claimed independent; only the impossible sibling-sum>combined diverges, NO arbitrary floor). The aggregate ownership **% has NO independent source** — every vendor sums the same SEC 13F/Form 4/13D filings + disagrees only by method → single-source by construction (documented in metrics-analyst, NOT a per-response field). Facts-not-gate; never changes a share count; pure `_classify_cross_check` table-tested, SQL readers dev-verified on the panel. | `app/services/ownership_rollup.py::_cross_validate_denominator` + `_classify_cross_check` + `_read_shares_outstanding_near` + `DenominatorCrossCheck`; spec `docs/specs/etl/2026-06-17-ownership-denominator-cross-check.md` (#1647 pt5); prevention-log "A derived aggregate has no independent cross-source — tie out to the primary source" |
| I20 | A multi-class issuer's **market cap** is the TOTAL company capitalization = Σ over share-class siblings (per-class FSDS shares × that sibling's price) + an untraded residual class imputed at the largest traded class's price — **identical on every sibling's page** (GOOG and GOOGL both = the one company, ~$4.45T). The legacy `combined_count × this-class-price` (`compute_market_cap`) is structurally wrong (one company → two different caps; prices all classes at one class's price); the per-class **float value** (Class A shares × price) is a DIFFERENT metric, never labelled "market cap". `resolve_market_cap_basis` → `total_company` (use it) / `multiclass_unavailable` (curated dual-class, no clean total → **suppress null**, never publish the broken product) / `not_multiclass` (legacy product, exact for genuine single-class). **Multi-class oracle = presence in the #1623 curated `instrument_class_shares_outstanding`** — NOT a raw shared-CIK sibling count (56 dev CIKs / 122 instruments, dominated by `.US` dups, ETF trusts, warrants, preferreds = the #1646-filtered junk). Fail-closed guards (residual ≤25% of combined, Σ-classes ≤ combined +0.5% else mismatch not residual, combined-near ≤400d delta, future-period reject, freshness/structural via shared `class_shares_usable`). Combined read falls back across siblings so a missing companyfacts fanout on one sibling can't break the identical-across-siblings invariant. PURE READ-PATH; **dilution stays combined** (growth-rate of total shares, not per-class). **Ranking-grade follow-up (#1664), DONE:** the `instrument_valuation.market_cap_live` VIEW (sql/080) carried the same legacy distortion (feeds `scoring.py` + the view's P/S, P/FCF, EV/EBITDA, P/B). A SQL view CANNOT reproduce the fail-closed total-company policy (per-class prices + residual imputation + guards) → **sql/201 takes the `multiclass_unavailable` posture for ALL view readers: SUPPRESS (NULL) every shares-distorted column** (`market_cap_live`, `enterprise_value`, `pb_ratio`, `price_sales`, `p_fcf_ratio`, `fcf_yield`, `ev_revenue`, `ev_ebitda`) for curated dual-class issuers (CIK ∈ `instrument_class_shares_outstanding`, lpad-10); KEEP the clean per-share/ratio columns (`pe_ratio`, `debt_equity_ratio` = debt/total-equity, margins, roa/roe, dividend_yield — no `shares×price` term). The decision-grade consumer (`scoring.py::_apply_market_cap_basis`) then OVERLAYS the correct total via `resolve_market_cap_basis` so the value score uses the right market cap / fcf_yield (= company TTM FCF / total-company cap) instead of a degraded NULL. | `app/services/xbrl_derived_stats.py::resolve_market_cap_basis` + `_assemble_total_company_cap` (pure) + `_build_total_company_cap`; `ownership_rollup.py::class_shares_usable` (shared gate); `app/services/scoring.py::_apply_market_cap_basis` + `sql/201` (#1664); spec `docs/specs/etl/2026-06-17-per-class-market-cap.md` (#1662) + `…2026-06-18-ranking-view-dual-class-market-cap.md` (#1664); prevention-log "Market cap of a multi-class issuer is the total company…" + "A SQL view cannot reproduce a fail-closed multi-step policy — suppress in the view, compute in the Python consumer that owns the policy" |
| I22 | **Total return** (#1635) is a SECOND CAGR in the risk_v1 evidence layer — price return + reinvested per-share dividends — feeding the scoring **v1.3 Calmar reward**. Source rule = the us-gaap per-share dividend/distribution CONCEPTS (reuse `sec_fundamentals.TRACKED_CONCEPTS["dps_declared"|"dps_cash_paid"]`, lazy import — covers LP/LLC REIT/MLP/BDC like O/MAIN that don't tag CommonStock). Facts are DURATION facts; tier = duration band (monthly 25-45d / quarterly 75-100d / annual 350-380d; 180/270d = YTD cumulatives, dropped). Stream build (`select_dividend_stream`, pure): modal-dedup per (group,start,end) → pick ONE dominant concept group (never mix declared+cash_paid; 308 dev instruments tag both) → HIGH-side outlier guard (>5× global per-tier median, two-pass) → **residual tier-fill** finest-first (`annual − Σ wholly-contained finer = implied Q4`; the dominant US 3×10-Q+10-K pattern omits a standalone Q4 — a plain reject-overlap loses it). `total_return_index` reinvests at first close ≥ ex-date≈period_end (sum same-date divs once); non-payer/empty stream → TR == price exactly. `tr_status` gate ({ok, tr_incomplete, no_dividends}): empty stream → `no_dividends` UNLESS `instrument_dividend_summary.has_dividend` is `True` (view rows only ever-payers → absence = never-payer; True+empty = payer we failed to ingest → `tr_incomplete`); else span-relative coverage anchored on first-EVER ex-date (initiation vs data-gap) + internal-gap (>1.5× tier) + terminal-staleness (>2× tier, floored at quarterly filing cadence for monthly). Additive-nullable under stable `risk_v1` (sql/203, NO version bump). Calibrated 3y tr_calmar tail p75≈0.74 / p90≈2.05 → reward HIGH=0.75 / EXTREME=2.0. | `app/services/risk_metrics.py` (`select_dividend_stream`, `total_return_index`, `tr_status`, `load_dividend_facts_bulk`, `load_has_dividend`, `_RISK_BUSINESS_COLS` tr_*); `app/services/scoring.py::_calmar_reward` + v1.3 modes; `sql/203`; spec `docs/specs/ranking/2026-06-19-sec-total-return-calmar-v1.3.md` (#1635 / #1633-vnext); prevention-log "Reconstructing a per-period series from SEC duration facts…" + "A summary VIEW that only rows matching entities encodes its answer in row ABSENCE…" |
| I23 | A stored thesis carries a **subject-identity verdict** and the deterministic layer reads the VERDICT, never a re-derivation. `theses.subject_identity_ok` / `_rule_version` / `_checked_at` move as a triple (all NULL = never checked, or all set — `sql/332` CHECK). ⚠⚠ **NULL is *not yet checked*, NOT *passed*** — every consumer tests `is True`, never `is not False`. The rule lives in its OWN module so `RULE_SET_VERSION` can be a hash of the rule alone (`thesis.py` changes for prompt/provider/schema work and would mark the corpus stale for nothing); edit it and the next boot re-verdicts all rows. ⚠ The populating path is the `app/main.py` lifespan self-heal, not a script an operator must remember: the migration can only add NULL columns, so without it a migrate-and-deploy silently strips the whole historical corpus out of portfolio/scoring/entry-timing/reporting. Quarantined ⇒ SKIP with a reason code (`thesis_quarantined`), never fall back to an older passing thesis. Nothing is deleted — the rows are the evidence base for the write-side fix. | `app/services/thesis_subject_identity.py` (rule + `is_thesis_usable` + `ensure_subject_identity_verdicts`); `sql/332`; consumers `portfolio.py:459`, `scoring.py:1299,1659`, `entry_timing.py:96`, `reporting.py:607`; `app/api/theses.py` exposes it, `app/api/alerts.py` suppresses both-sides-quarantined change pairs; spec `docs/proposals/thesis/2026-08-12-thesis-subject-identity-quarantine.md` (#2436, #2431); prevention-log "A fail-closed read guard turns every existing row, and every fixture, into the refused path" |
| I21 | The DEF 14A `def14a_unmatched` slice is a **non-additive memo overlay** (`denominator_basis="proxy_disclosure"`), NOT a pie wedge (#1659 — **reverses #1627**). DEF 14A "Security Ownership of Certain Beneficial Owners" is a Rule 13d-3 deemed/overlapping disclosure (SEC Item 403: *"where more than one beneficial owner is listed for the same securities, disclose to avoid confusion"*) — the same shares listed under multiple owners (control groups, parent/sub, spouse attribution, "all officers as a group" aggregates). The real holders are already counted + de-duplicated via 13D/G, 13F, Form 4; the unmatched proxy residual is overwhelmingly deemed-blocks/aggregates/parser-fragments, almost never *new additive* shares. The disambiguating footnote ("also deemed to own shares held by X") is **free-text prose** — not structured in EDGAR/edgartools/our parser — so NO read-path heuristic can safely cluster it (a name-keyed exact-value collapse wrongly merges independent equal-grant executives: dev FPs BXP Koop+LaBelle @1,875,000, FOXA Ciongoli+Tomsic @2,750,000; CIK absent, `holder_role` doesn't separate them). So demote, don't cluster: all additive math (residual/concentration/sanity/per-class-denominator guard) filters `denominator_basis == "pie_wedge"` → def14a auto-excluded; all memo paths (CSV split, FE overlay) filter `!= "pie_wedge"`. Proxy holders stay inspectable (L1 overlay, L2 "DEF 14A (memo)" drilldown, CSV memo scope). Dissolves the WHOLE proxy double-count class (#1644 family / #1645 / #1652 were per-channel patches; this kills it at the source). | `app/services/ownership_rollup.py` def14a slice `denominator_basis="proxy_disclosure"` + `DenominatorBasis` Literal; API `_OwnershipSliceModel` + FE `OwnershipDenominatorBasis`; spec `docs/specs/etl/2026-06-17-def14a-nonadditive-overlay.md` (#1659); data-eng I14/I16; prevention-log "A source that lists the same shares under multiple owners (deemed/overlapping) is not additive — demote the whole source, don't heuristically de-dup it" |
...
| `unresolved_13f_cusips` | sql/099 + sql/164 + sql/189 | Resolver work-queue for SEC-supplied CUSIPs not yet resolved to instruments. **Two-writer split** (#1233 PR-1a): legacy per-filing path (`source IS NULL`, partial UNIQUE `..._legacy_idx`) carries `name_of_issuer` + `last_accession_number`; bulk-dataset path (`source IN ('bulk_13f_dataset','bulk_nport_dataset')`, partial UNIQUE `..._bulk_idx` on `(cusip, source)`) is **per-(cusip, source) grain since #1349 (sql/189)** — aggregate `observation_count` (best-effort heuristic) + `first_period_end`/`last_period_end` range, issuer name NULL until the OpenFIGI sweep (PR-1b) fills it. Hygiene: `sweep_bulk_cusips_resolved_via_extid` tombstones mapped cusips (replaced #1399 inline delete); retention purge drops rows whose `last_period_end` aged past the per-source floor; writers gate `period_end >= cutoff` so the table is in-window by construction |
...
* **OpenFIGI sweep** (#1233 PR-1b; drain + cadence #740): `cusip_resolver.sweep_unresolved_cusips_via_openfigi` reads bulk-source rows whose CUSIP has no existing `external_identifiers` row, calls `OpenFigiResolver` (host `api.openfigi.com`, own Lane), normalises the returned ticker (`BRK/B`→`BRK.B` — OpenFIGI uses slash share-class convention, probed 2026-06-11), and on success writes `external_identifiers (provider='openfigi', identifier_type='cusip', is_primary=FALSE)`. The bulk rows are tombstoned with `resolution_status='resolved_via_openfigi'`. **Negative verdicts are tombstoned too** (#740, sql/192): `openfigi_unknown` (API succeeded, no US-common-stock mapping) and `openfigi_no_instrument` (ticker not in / ambiguous within the universe) — terminal in v1, manual `SET resolution_status=NULL` is the reset escape hatch; transport/429 leaves NULL (retried). Pre-#740 negatives stayed NULL and the `ORDER BY cusip LIMIT 1000` selection re-scanned the same alphabet head forever (54k backlog, 2 net promotions/run). Runs as bootstrap stage S13 AND weekly Sun 06:00 UTC ScheduledJob (dual-registered, both on lane `openfigi`); the invoker drains with tier-derived `max_passes` (keyed 60 / unkeyed 3). Lane: `openfigi` (cap=1; per-instance rate limiter at `app/services/openfigi_resolver.py`). Tier-dependent budget — unkeyed 250 mappings/min; keyed 25k mappings/min via `OPENFIGI_API_KEY`. SD-1 cross-reference (`docs/settled-decisions.md`) gates programmatic use. Spec: `docs/specs/etl/2026-06-11-openfigi-sweep-negative-status.md`. **After a big drain, observations for newly-mapped CUSIPs need a bulk re-ingest** (`sec_13f_ingest_from_dataset` / `sec_nport_ingest_from_dataset`) — the mapping alone doesn't materialise history.
...
| `n_port_ingest._resolve_cusip_to_instrument_id:696` | **yes, as of #2329** (per-filing N-PORT ingest + the `sec_n_port` manifest parser) |
| `ownership_drillthrough._blockholder_state:263, :281` | **yes, as of #2329** — narrow until then, in the same function whose 13F sibling PR-1b widened eight lines above |
| `blockholders._resolve_cusip_to_instrument_id` | **deleted at #2329** — no callers since #1628 (`c4f1d2e9`) replaced it with `_resolve_issuer_to_instrument_id:450` |

⚠⚠ **#2329 — "latent" is a claim about the population, not a property of the code.** Both sites above were filed as latent-and-harmless because each had a provider-wide **bulk twin** said to cover whatever the narrow copy dropped. Measured 2026-09-19 (`PYTHONPATH=. uv run python -m scripts.audit_2329_cusip_consumer_widening`, which recomputes every figure here): the N-PORT one was dropping **348 of 1,025 eligible holdings** across the FULL population of parsed accessions, and **221** of the resulting `(accession, instrument)` pairs were absent from `ownership_funds_observations` — live loss, not latency. The twin argument was refutable without a query: **DERA publishes N-PORT quarterly and the per-filing path exists to handle a filing the week it lands, so a twin on a slower cadence covers the middle of the distribution and none of the recent edge.** State the LAG between two paths before accepting "the other one covers it". And the 13D/G one was not latent either — it was **dead**, superseded at #1628, so the correct change was deletion; grep the callers before widening, because "fix it everywhere" and "delete it" look identical from a call-site table.
...
**Insider (Form 3/4/5) writer routing — route by the PARSED issuer CIK, never the discovery instrument (#828 PR-1).** EDGAR indexes each ownership document under BOTH the issuer's and every reporting owner's CIK stream, so discovery keyed on the OWNER's feed binds the accession to the owner's instrument (BAC Form 4 under BRK.B). The apply chokepoints (`insider_transactions.upsert_filing`, `insider_form3_ingest.upsert_form_3_filing`) resolve `parsed.issuer_cik` via `sec_identity.resolve_insider_writer_routing`: discovery instrument outside a non-empty `siblings_for_issuer_cik` set → entity rows bind to the interim pick (unambiguous `instrument_cik_history` instrument inside the set, else `min(siblings)`), observations fan across the issuer siblings, and `filings.reconcile_mislinked_ownership_filing_events` deletes the owner's `filing_events` bindings + upserts issuer-sibling rows (the fe bridge row IS the L2 pollution). The fe writers (`_upsert_filing` / `_upsert_filing_event`) additionally guard ownership forms: once the accession is parsed with a routable issuer, a non-sibling binding is refused — this closes the issuer-parses-first / owner-stream-walks-later ordering and full re-walk re-pollution. Empty/garbage sibling set fails open (discovery linkage kept — the unroutable cohort). Rewash/backfill scripts must go through the same chokepoints (prev-log "per-instrument-scoped rewash… must mirror live-ingest sibling fan-out", #2105).
...
- `sec_rebuild.py` — `POST /jobs/sec_rebuild/run` (operator-triggered targeted re-ingest). **⚠ Re-FETCHES SEC**: resetting manifest rows to `pending` routes them through `sec_manifest_worker` → the per-source parser, which calls `provider.fetch_document_text(...)` for EVERY row (verified `manifest_parsers/sec_13dg.py:182`). So a universe-scope re-drain (`{"source":"sec_13d"}`) on a **parser-version bump** = N rate-limited SEC fetches (13D/G ≈138k → hours, starves other ingest, and leaves affected `_current` empty until each row drains). For a parser-version bump where the raw body is already STORED (`filing_raw_documents`), prefer the **rewash pattern — re-parse the stored raw LOCALLY** (no SEC): either `rewash_filings` specs, or a one-off that feeds each accession's stored payload back through the same adapter + `record_*_observation` + `refresh_*_current` (#1638 backfill did exactly this for 826 instruments in minutes after `sec_rebuild` proved too slow). Use `sec_rebuild` proper only when raw is ABSENT or you need discovery of missing accessions (`discover=True`). Scope to affected instruments, not the universe, whenever you can enumerate them.
...
### Q16. Why is GOOGL's ownership % "a combined-basis lower bound" (the dual-class caveat)? (#1646)

GOOG and GOOGL share Alphabet's SEC CIK 1652044. The rollup numerators are per-class-correct (institutions/insiders/blockholders resolve via CUSIP — GOOG `02079K107` ≠ GOOGL `02079K305`), but the **denominator** is Alphabet's COMBINED all-class `shares_outstanding` (~12.1B), because the per-class `dei:EntityCommonStockSharesOutstanding` is dimensional (`StatementClassOfStockAxis` member) and **stripped by the companyfacts API** (sec-edgar §7.17), so `financial_facts_raw` never receives it. Result: GOOGL institutions 2.54B ÷ 12.1B = 20.97% renders, but the true per-class figure is 2.54B ÷ 5.82B Class A ≈ 43.7%.

Until the per-class denominator lands (#1590 DERA FSDS `segments`), the rollup ships an **honest caveat, not a fabricated split**: `rollup.dual_class_denominator` (a `DualClassDenominator` with `cik`, `sibling_symbols`, server-owned `note`) flags that every percentage is a combined-basis lower bound. The FE renders the `note` as a callout below the concentration chip; the CSV emits one inert `__dual_class_denominator__` memo row. It is NOT a `CorrectionApplied` (removes no shares). Detection = invariant I18's three gates (`_detect_dual_class_denominator`). Single-class issuers, ETF/ETN trusts, and `.US` dual-listings are excluded by the us-gaap-denominator gate. A multi-class issuer with only one class in the universe (BRK.B — BRK.A absent) can't be caught by the sibling test and stays silently understated until #1590.
...
- ⚠⚠ **`price_daily` is populated for 41% of the universe and FRESH for 40.5% (5,140 of 12,684) — but was fresh for only 11% until #2254 landed on 2026-08-04.** `daily_candle_refresh` (`app/workers/scheduler.py::daily_candle_refresh`) refreshes held ∪ T1/T2 ∪ `BENCHMARK_SYMBOLS` ∪ T3 from `_T3_CANDLE_SELECT`. That T3 query used to be **seed-only** — it carried `NOT EXISTS (price_daily)`, selecting on the absence of the rows it writes, so a Tier-3 instrument left refresh scope on its first bar and nothing took over unless it promoted (`total_score >= 0.55`) or was held/benchmark; 3,523 of 3,838 priced T3 had no bar in 30 days and every crypto / FX / commodity series was ~2 months stale. It now carries **two arms**: a seed arm (unpriced ∧ eligible, unchanged #515 gate) and a **maintenance arm** (priced ∧ `last_bar < most_recent_trading_day`, deliberately **no** eligibility gate — applying the seed gate there would have stranded 274 priced `us_equity` T3 with no `fundamentals_snapshot` row). Consequences for any new consumer: (1) **never assume `EXISTS(price_daily)` means usable** — check `max(price_date)`; the two denominators still differ and the next scope regression will look identical from the existence side; (2) **coverage tier is implicitly US-only**, because it is driven by `fundamentals_snapshot`, which is SEC-fed, so all 1,383 T1/T2 instruments are `us_equity` and no non-US instrument has ever reached T1/T2. Scoping a market-wide surface on "scoring-eligible" therefore silently means "US filer". eToro *does* serve daily candles for the international set (27/28 probed, current to the prior close) — the 7,463 unpriced instruments are our selection gate, not provider supply. (3) **~108 instruments are supply-less, not unrefreshed**: 81 priced ones return HTTP 200 with no advance (78 `us_equity`, oldest last bar 2021-05-21) plus the 27 unpriced gate-passers that return no bars. The provider does not error on these, so nothing distinguishes them from "not yet refreshed" until a marker exists — see TA design doc §3 decision 9.
...
**Independent-doc-chain prefetch — the `prefetch_chain` hook (#1730).** The pass-1 `fetch_url` → pass-2 `expand_urls` mechanism only reaches docs discoverable FROM the pass-1 body. A parser that fetches a SECOND doc-chain discovered from a DIFFERENT doc (the 10-K's XBRL linkbases — `index.json` → `discover_xbrl_files` → instance/label/def, independent of the primary HTML) can't be reached that way; making `index.json` the pass-1 doc would regress primary-HTML prefetch for a no-XBRL filing (pass-2 is skipped on a pass-1 miss). The fix is the `prefetch_chain` hook: a self-contained per-source prefetch that runs its OWN concurrent `fetch_document_texts` rounds (same shared ≤10 req/s throttle) and returns `{url: body}` merged into the tick cache. Load-bearing rules:
- **Share the URL/discovery construction with the serial path, never duplicate it.** A one-character URL mismatch silently misses the cache (no wrong data, just no speedup — the worst kind of bug: invisible). #1730 extracts one `archive_dir_url` builder (routed through by `fetch_filing_index` + the serial dimensional fetch + the chain) and one `_xbrl_index_locator`. The chain pre-populates the cache; the serial path RE-discovers roles (instance vs label vs def) itself from the cached index — so a flat URL→body cache is sufficient and no structured role info crosses the hook.
- **Make the discovery-index fetch cache-aware too.** `fetch_filing_index` returns parsed JSON, not text, so it does NOT consult the body cache by default — without a consult the index round stays serial and defeats the prefetch. #1730 makes it parse a cached index text (guard malformed → live fetch). Scoped: zero change outside a worker tick; inside a tick a same-URL caller gets the cached (same immutable) doc.
- `_prefetch_bodies` must run chains EVEN when no source registered a `fetch_url` (the `pass1_url_to_rows`-empty early-return must not skip a chain-only source). Best-effort: a chain raise logs + skips (serial parser re-fetches), never aborts the tick. Artifacts are parse-and-drop (#470) → pure concurrency, no reuse, no #1617 bucket.
