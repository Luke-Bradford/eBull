...
Last audit pass: 2026-05-24 (Stream A PR-D landed; Layer 1/2/3 wiring shipped under #1155 / PR #1157 on 2026-05-13; `_BOOTSTRAP_STAGE_SPECS` now at 27 stages with S25 `fundamentals_sync_bootstrap` (Stream A PR-C2) / S26 `mf_directory_sync` / S27 `sec_n_csr_bootstrap_drain`). Previous audit 2026-05-14 (post #1168 / sec_10q synth no-op parser shipped — closes G4).
...
|---|---|---|---|
| 1 | **Bootstrap stage** — first-install drain | `_BOOTSTRAP_STAGE_SPECS` | `app/services/bootstrap_orchestrator.py:1035-1193` (27 stages; #1174 added S26 `mf_directory_sync` + S27 `sec_n_csr_bootstrap_drain`; Stream A PR-C2 / #1310 made S25 = `fundamentals_sync_bootstrap` derivation-only entrypoint) |
...
| `sec_n_csr` | S26 `mf_directory_sync` (classId resolver inputs) + S27 `sec_n_csr_bootstrap_drain` (last-2y per-trust manifest enqueue) | manifest worker (real fund-metadata parser, #1171) | 200d | both | `sec_rate` | ✅ `sec_n_csr.py` (#1171, 2026-05-15, replaced #918 / PR #1170 synth no-op) | **WIRED** — real fund-metadata parser + dedicated bootstrap drain landed in #1174. Extracts per-(series, class) Tier 1 scalars (expense ratio, NAV, returns, portfolio turnover, holdings count, inception, contact) + Tier 2 dimensional JSONB (sector / region / credit allocation, returns_pct, benchmark_returns_pct, growth_curve) + Tier 3 `raw_facts` fallback. `requires_raw_payload=False` (no `store_raw`; re-parse re-fetches iXBRL companion). Source-priority chain: `period_end DESC, filed_at DESC, source_accession DESC` (settled-decisions §"Source priority for fund metadata"). classId → instrument_id via `external_identifiers (provider='sec', identifier_type='class_id')` populated by the dedicated S26 `mf_directory_sync` bootstrap stage (#1174; daily cron `daily_cik_refresh` retains the bundled call as a drift-heal safety net). S27 `sec_n_csr_bootstrap_drain` walks distinct trust CIKs from `cik_refresh_mf_directory` and enqueues last-2-years N-CSR + N-CSRS accessions with `subject_type='institutional_filer' + instrument_id=NULL` for the manifest worker to drain. Spike `docs/_archive/2026-05/spike-n-csr-feasibility.md` INFEASIBLE-CONFIRMED verdict applied to holdings-attestation lane only (§10.6 scope narrowing); fund-metadata lane is orthogonal. |
| `sec_xbrl_facts` | Stage 9 `sec_companyfacts_ingest` (bulk-zip) + Stage 24 `fundamentals_sync` | `JOB_FUNDAMENTALS_SYNC` cron (`scheduler.py:616`) + manifest worker (synth no-op, G7) | 120d | both | `sec_rate` (unused by parser) | ✅ `sec_xbrl_facts.py` (G7) | **WIRED** — synth no-op (sec-edgar §11.5.1). XBRL facts land via the Companyfacts bulk JSON path; the manifest row IS the audit signal. Parser body is `return ParseOutcome(status='parsed', parser_version='xbrl-facts-noop-v1')`. Closes G7. |
...
|---|---|---|---|---|---|
| 1 | Atom `getcurrent?action=getcurrent&output=atom` (every 5 min) | `run_atom_fast_lane` at `app/jobs/sec_atom_fast_lane.py:104` | — | `sec_atom_fast_lane` ScheduledJob (5 min cadence, `scheduler.py:1051`) | ✅ **WIRED 2026-05-13 (#1155 / PR #1157)** — `_INVOKERS[JOB_SEC_ATOM_FAST_LANE]` at `runtime.py:333`; ScheduledJob at `scheduler.py:1051` |
| 2 | Daily `master.YYYYMMDD.idx` (04:00 UTC reconciliation) | `run_daily_index_reconcile` at `app/jobs/sec_daily_index_reconcile.py:46` | — | `sec_daily_index_reconcile` ScheduledJob (daily 04:00 UTC, `scheduler.py:1072`) | ✅ **WIRED 2026-05-13 (#1155 / PR #1157)** — `_INVOKERS[JOB_SEC_DAILY_INDEX_RECONCILE]` at `runtime.py:334`; ScheduledJob at `scheduler.py:1072`; exempt from universal-bootstrap gate (#1181) |
| 3 | Per-CIK `submissions/CIK*.json` (per `data_freshness._CADENCE`) | `run_per_cik_poll` at `app/jobs/sec_per_cik_poll.py:39` | — | `sec_per_cik_poll` ScheduledJob (hourly :00, `scheduler.py:1103`) | ✅ **WIRED 2026-05-13 (#1155 / PR #1157)** — `_INVOKERS[JOB_SEC_PER_CIK_POLL]` at `runtime.py:335`; ScheduledJob at `scheduler.py:1103` |
...
Tickets #867 / #868 / #870 CLOSED 2026-05-13 by **#1155 / PR #1157** which added the missing `_INVOKERS[]` + `ScheduledJob` rows. Wiring CONFIRMED LIVE via `grep` on `app/jobs/runtime.py:333-335` + `app/workers/scheduler.py:1051, 1072, 1103`.
...
|---|---|---|---|---|---|
| `www.sec.gov/files/company_tickers.json` | `app/providers/implementations/sec_edgar.py:52` | Stage 6 `cik_refresh` (`JOB_DAILY_CIK_REFRESH`) | `JOB_FUNDAMENTALS_SYNC` daily (`scheduler.py:616`) calls `daily_cik_refresh()` inline at `scheduler.py:3051` | `sec_rate` | Ticker → CIK bridge, ~10k operating-co rows; conditional GET ETag-aware. **Note:** `JOB_DAILY_CIK_REFRESH` is in `_INVOKERS` (manual trigger only) but absent from `SCHEDULED_JOBS` — the daily cadence lives inside the fundamentals_sync body, not as a standalone scheduled job. |
...
`_BOOTSTRAP_STAGE_SPECS` at `app/services/bootstrap_orchestrator.py:1035-1193`. **27 stages** (#1174 added S26 `mf_directory_sync` + S27 `sec_n_csr_bootstrap_drain`; Stream A PR-C2 / #1310 made S25 = `fundamentals_sync_bootstrap` derivation-only entrypoint), 5 base rate lanes (`init`, `etoro`, `sec_rate`, `sec_bulk_download`, `db`) + post-#1141 family split (`db_filings`, `db_fundamentals_raw`, `db_ownership_inst`, `db_ownership_insider`, `db_ownership_funds`) + post-PR-1b `openfigi` lane + post-#915 `finra` lane. See `.claude/skills/data-engineer/SKILL.md` §6.5.1 for the full lane catalogue + cap rules.
...
| 16 | `sec_first_install_drain` | sec_rate | `sec_first_install_drain` (`max_subjects=None`) | Per-CIK submissions.json deep-history walk (cohort drops 11k→3-5k post-#1010/#1222) |
| 17 | `sec_def14a_bootstrap` | sec_rate | `sec_def14a_bootstrap` | DEF 14A backfill |
| 18 | `sec_business_summary_bootstrap` | sec_rate | `sec_business_summary_bootstrap` | 10-K Item 1 backfill |
| 19 | `sec_insider_transactions_backfill` | sec_rate | `sec_insider_transactions_backfill` | Form 4 backfill |
| 20 | `sec_form3_ingest` | sec_rate | `sec_form3_ingest` | Form 3 |
| 21 | `sec_8k_events_ingest` | sec_rate | `sec_8k_events_ingest` | 8-K |
| 22 | `sec_13f_recent_sweep` | sec_rate | `sec_13f_quarterly_sweep` (`min_period_of_report=<dynamic:bootstrap_13f_cutoff>`; `min_last_13f_hr_at=<dynamic:bootstrap_13f_hr_cutoff>`) | 13F recent sweep (cohort-bounded per #1010) |
| 23 | `sec_n_port_ingest` | sec_rate | `sec_n_port_ingest` (`min_last_seen_filed_at=<dynamic:bootstrap_nport_cutoff>`) | N-PORT |
| 24 | `ownership_observations_backfill` | db | `ownership_observations_backfill` | Recompute `_current` from observations |
| 25 | `fundamentals_sync` | db_fundamentals_raw | `fundamentals_sync_bootstrap` (Stream A PR-C2 / #1310 — derivation-only; NO HTTP) | Derive financial periods + TTM from already-loaded `financial_facts_raw` (S9-populated). 4-cap req (bulk_archives_ready + cik_mapping_ready + submissions_processed + fundamentals_raw_seeded) per PR-C1 / #1309. |
| 26 | `mf_directory_sync` | sec_rate | `mf_directory_sync` | classId → instrument_id mapping + fund-trust directory refresh (#1174); provides `class_id_mapping_ready` for S27 |
| 27 | `sec_n_csr_bootstrap_drain` | sec_rate | `sec_n_csr_bootstrap_drain` | Last-2y per-trust N-CSR / N-CSRS manifest enqueue. Terminal stage. |
...
|---|---|---|---|---|
| G1 | Layer 1 Atom fast-lane unwired | ✅ CLOSED 2026-05-13 | **#867 / #1155** (PR #1157) | `_INVOKERS[JOB_SEC_ATOM_FAST_LANE]` at `runtime.py:333` + ScheduledJob at `scheduler.py:1051` (5 min cadence) |
| G2 | Layer 2 daily-index reconcile unwired | ✅ CLOSED 2026-05-13 | **#868 / #1155** (PR #1157) | `_INVOKERS[JOB_SEC_DAILY_INDEX_RECONCILE]` at `runtime.py:334` + ScheduledJob at `scheduler.py:1072` (daily 04:00 UTC; #1181 carve-out exempts from universal-bootstrap gate) |
| G3 | Layer 3 per-CIK poll unwired | ✅ CLOSED 2026-05-13 | **#870 / #1155** (PR #1157) | `_INVOKERS[JOB_SEC_PER_CIK_POLL]` at `runtime.py:335` + ScheduledJob at `scheduler.py:1103` (hourly :00) |
