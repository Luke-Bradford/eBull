...
| `sec_n_csr` | — | manifest worker (synth no-op per #918) | 200d | both | `sec_rate` (unused by parser) | ✅ `sec_n_csr.py` (#918, 2026-05-14) | **WIRED** — synth no-op (sec-edgar §11.5.1, second exemplar after sec_10q). Spike `docs/superpowers/spikes/2026-05-14-n-csr-feasibility.md` confirmed INFEASIBLE for v1 by raw-payload inspection across 3 active families: zero CUSIP/ISIN/SEDOL/ticker/portfolio-CIK in any of 4 iXBRL companions or 52 MB of primary HTML. Fund holdings land via N-PORT-P already. Closes G5. |
