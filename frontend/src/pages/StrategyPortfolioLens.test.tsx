...
const CORE_COLLECTING = {
  ...
  household_currency_caveat:
    "Sterling is not the unit here. Where the core sleeve holds a USD-quoted instrument, a household measuring it in GBP carries GBP/USD exposure on the whole position value and not only on its return, and this engine does not hedge it. A £ sign in the positions table is the broker's own label carried through unchanged — no amount on this page has been converted by us. Converting when you fund or withdraw is a household cost that the preregistered ceiling, which prices round-trip spread, does not include.",
} as const;
